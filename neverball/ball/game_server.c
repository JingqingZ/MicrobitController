...
static void input_set_x(float x)
{
  ...
  if( fd == -1){ 
     fd = open(MICROBIT_PATH, O_RDWR);
  }
  FILE* fp = fdopen(fd,"r");


   //if(fd==NULL){fd = fopen(MICROBIT_PATH, "r");}
...
static void input_set_r(float r)
{
  ...
  FILE* fp = fdopen(fd,"r");
   ...
   smooth_rot_i = (smooth_rot_i+1)%5;
   smooth_rot[smooth_rot_i]=r;
   ...
   //printf("%f\n", r);
    ...
    //input_current.r = current_rot - r/5;
    //current_rot = r/5;
    
    input_current.r = current_rot - r;
    current_rot = r;
   // l+=1;
   // input_current.r = current_rot - l;
    //current_rot = l;
    //printf("%f\n", l);
   // if (l==90){l=0;input_current.r=-90;}
    //input_current.r =r;
    //input_current.r =r;
}
